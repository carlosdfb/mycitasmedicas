...
class ChartController extends Controller
{
    ...
    public function doctorsJson(Request $request)
    {
        ...
        $doctors = User::doctors()
            ->select(['name'])
            ->withCount(['attendendAppointments' => function ($query) use ($start, $end) {
            ...
            /*            ->withCount('attendendAppointments')
                        ->withCount('cancelledAppointments')*/
            ->orderBy('attendend_appointments_count', 'desc')
            ->take(3)
            ->get();
        $data = [];
        $series = [];
        ...
        $series1['data'] = $doctors->pluck('attendend_appointments_count'); // atendidas
